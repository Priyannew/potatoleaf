...
def home():
    if request.method == "POST":
        ...
        print(pred)
        return render_template('home.html', pred=pred[0], input_img=input_img, pred_img='pred_img.png',x=list(pred[1:]))
...
def predict_save(img):
    ...
    colors = ['green' if prob < threshold_low else 'orange' if prob < threshold_high else 'red' for prob in out]
    ...
    return [class_names[np.argmax(out)],out]
